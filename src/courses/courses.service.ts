...
export class CoursesService {
    constructor(
        ...
        private readonly tabRepository: Repository<Tag>
    ) {}
    ...
    async findOne(id: number) {
        ...
        if (!course) {
            throw new NotFoundException(`Course id ${id} not afound`)
        }
    ...
    async preloadTagByName(name: string): Promise<Tag> {
        const tag = await this.tabRepository.findOne({where: {name}})
        if (tag) {
            return tag
        }
        return this.tabRepository.create({name})
    }
}
